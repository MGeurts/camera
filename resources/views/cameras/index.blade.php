@extends('layouts.app')
@section('title', 'Camera Dashboard — HIKVISION')
...
.camera-card:hover { border-color: var(--accent-dim); box-shadow: 0 0 18px var(--accent-glow); }
...
.rate-select:hover, .rate-select:focus { border-color: var(--accent); color: var(--accent); }
...
.empty-state h2 { font-family: var(--font-display); font-size: 26px; letter-spacing: 4px; margin-bottom: 12px; color: var(--text-muted); }
.empty-state p { font-size: 11px; line-height: 1.8; }
.empty-state code { background: var(--bg-input); border: 1px solid var(--border); padding: 1px 7px; border-radius: 3px; color: var(--accent); font-family: var(--font-mono); font-size: 11px; }

/* ═══════════════════════════════════════════════════════════
   FULLSCREEN OVERLAY
   Sits above everything. Uses the Browser Fullscreen API so
   the OS chrome (taskbar etc.) also disappears.
═══════════════════════════════════════════════════════════ */
#fs-overlay {
    display: none;
...
/* Camera label — bottom left, fades out */
.fs-label {
    position: absolute; bottom: 6px; left: 8px; z-index: 4;
...
/* Quit button — fixed bottom-right, fades in/out with mouse movement */
#fs-quit {
...
#fs-quit:hover { border-color: #e02040; color: #ff4466; }
#fs-quit.visible { opacity: 1; }
</style>
@endsection
...
/* ══════════════════════════════════════════════════════
   COLUMN SOLVER  (shared by normal + fullscreen grid)
══════════════════════════════════════════════════════ */
...
function refreshFeed(id) {
    ...
    back.onload = () => {
        ...
        updateTimestamp(id);
        // Mirror to fullscreen cell if active
...
function markOnline(id) {
    const el = document.getElementById(`status-${id}`); const card = document.getElementById(`card-${id}`);
...
/* ══════════════════════════════════════════════════════
   FULLSCREEN
   - Builds a separate grid of <div.fs-cell> elements,
     each with its own double-buffer pair of <img> tags.
   - Uses the Browser Fullscreen API (F11-style) so the
     OS taskbar disappears too.
   - The normal refresh timers already call syncFsCell()
     on every successful frame, so the FS images stay in
     sync without any extra timers.
   - The "Quit" button fades in on mouse move and hides
     after 2.5 s of inactivity.
══════════════════════════════════════════════════════ */
let fsActive    = false;
let fsHideTimer = null;
const fsSlot    = {}; // active buffer slot per camera in FS grid
...
function buildFsGrid() {
    ...
    CAMERAS.forEach(cam => {
        ...
        // Seed with whatever the normal grid is currently showing
...
function layoutFsGrid() {
    ...
    // No info bar in fullscreen — pass 0 for infoH
    const cols   = calcOptimalCols(availW, availH, CAM_COUNT, 0);
    grid.style.setProperty('--cols', cols);
}

/* Called by refreshFeed() every time a new frame arrives */
...
function enterFullscreen() {
    ...
    // Show quit button briefly on enter, then on mouse move
    showQuitBtn();
...
// Also handle the user pressing Escape / F11 to exit browser fullscreen
