...
.breadcrumb { font-size: 10px; color: var(--text-muted); letter-spacing: 1px; }
...
.feed-hud { position: absolute; inset: 0; pointer-events: none; z-index: 5; }
.hud-tag {
...
.info-label { color: var(--text-secondary); letter-spacing: 0.5px; flex-shrink: 0; margin-right: 12px; }
.info-value { color: var(--text-primary); text-align: right; word-break: break-all; font-weight: 500; }
.info-value.accent { color: var(--accent); }
.info-value.ok { color: var(--ok-text); }
.info-value.danger { color: var(--danger-text); }

.controls-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 7px; padding: 10px; }
.ctrl-btn {
    background: var(--bg-input); border: 1px solid var(--border);
    color: var(--text-secondary); padding: 9px; border-radius: 3px;
    ...
    <div style="display:flex;flex-direction:column;gap:0;">
        <div class="feed-wrap">
            <img class="feed-img" id="feed-a"
                 ...
                 alt="{{ $camera['name'] }}" style="z-index:2; position:relative;"
            />
        ...
        <div class="info-panel">
            ...
            <div class="panel-body">
                ...
                <div class="info-row">
                    ...
                    <span class="info-value">{{ $refresh / 1000 }}s</span>
                </div>
                <div class="info-row">
...
async function loadDeviceInfo() {
    ...
    body.innerHTML = `<div class="info-row"><span class="info-label"><span class="spinner-inline"></span>Loading…</span></div>`;
    try {
        const data = await fetch(`/api/cameras/${CAM_ID}/info`).then(r => r.json());
        if (data.error) {
            body.innerHTML = `<div class="info-row"><span class="info-value danger">${data.error}</span></div>`;
            return;
        }
        body.innerHTML = Object.entries(data).map(([k,v]) =>
            `<div class="info-row">
                <span class="info-label">${k.replace(/_/g,' ')}</span>
                <span class="info-value">${v}</span>
            </div>`
        ).join('');
    } catch(e) {
        body.innerHTML = `<div class="info-row"><span class="info-value danger">Could not reach camera</span></div>`;
    }
}
